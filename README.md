...
Open datasets and the harness behind them, covering **image compression (JPEG / WebP / AVIF / JPEG XL), the HEIC-to-JPG size tax, cloud storage $/GB, and phone storage capacity.** Every number here is generated by a script in [`benchmarks/`](benchmarks/) that you can re-run yourself, on your own images, in one command.
